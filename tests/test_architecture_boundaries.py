...
def test_runtime_catalog_updates_do_not_execute_git():
    source = (ROOT / "services" / "extra.py").read_text(encoding="utf-8")
    assert "subprocess" not in source
    assert "git_commit_and_push" not in source
    handlers = (ROOT / "handlers" / "message_handlers.py").read_text(encoding="utf-8")
    assert "from services.catalog_service import add_ship" in handlers
